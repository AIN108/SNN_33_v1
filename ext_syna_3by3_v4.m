...
hold on;
    % Get the current synapse map for this subplot
    current_map = reshape(synapse_map(:, k), 3, 3)';
    
    % Loop through the elements in the map and add text annotations
    for i = 1:size(current_map, 1)
        for j = 1:size(current_map, 2)
            % Place a text annotation at position (j, i) with the weight value = 중앙에 웨이트값 배치 
            % Convert the value to string and control decimal points using
            % num2str = 2자리까지 
            text(j, i, num2str(current_map(i, j), '%.2f'), 'Color', 'red', 'HorizontalAlignment', 'center');
        end
    end
